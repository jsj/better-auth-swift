...
struct BetterAuthSessionMaterializer {
    ...
    func materializeSession(token: String, fallbackUser: TwoFactorUser) async throws -> BetterAuthSession {
        let session = try await context.refreshService.fetchCurrentSession(accessToken: token)
...
struct BetterAuthSessionResultHandler {
    ...
    func apply(_ outcome: BetterAuthSessionOutcome) async throws -> BetterAuthSessionOutcomeResult {
        ...
        case let .signedIn(session):
            return await .signedIn(try apply(session, event: .signedIn))

        case let .refreshed(session):
            return await .signedIn(try apply(session, event: .tokenRefreshed))
        ...
        case let .updatedUser(user, currentSession):
            ...
            return await .updated(try apply(BetterAuthSession(session: currentSession.session,
    ...
    private func apply(_ session: BetterAuthSession, event: AuthChangeEvent) async throws -> BetterAuthSession {
        _ = try await relay.setSession(session, event: event)
        return session
    }
}
