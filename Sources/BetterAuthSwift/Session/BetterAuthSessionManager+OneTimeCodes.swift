...
public extension BetterAuthSessionManager {
    ...
    func requestEmailOTP(_ payload: EmailOTPRequest) async throws -> Bool {
        try await throttleAuthOperation("requestEmailOTP")
        return try await makeOneTimeCodeService().requestEmailOTP(payload)
    }

    @discardableResult
    func signInWithEmailOTP(_ payload: EmailOTPSignInRequest) async throws -> BetterAuthSession {
        try await throttleAuthOperation("signInWithEmailOTP")
        return try await makeOneTimeCodeService().signInWithEmailOTP(payload)
    }

    @discardableResult
    func verifyEmailOTP(_ payload: EmailOTPVerifyRequest) async throws -> EmailOTPVerifyResult {
        try await throttleAuthOperation("verifyEmailOTP")
        return try await makeOneTimeCodeService().verifyEmailOTP(payload, currentSession: state.currentSession)
    }
    ...
    func requestPhoneOTP(_ payload: PhoneOTPRequest) async throws -> Bool {
        try await throttleAuthOperation("requestPhoneOTP")
        return try await makeOneTimeCodeService().requestPhoneOTP(payload)
    }

    @discardableResult
    func verifyPhoneNumber(_ payload: PhoneOTPVerifyRequest) async throws -> PhoneOTPVerifyResponse {
        try await throttleAuthOperation("verifyPhoneNumber")
        return try await makeOneTimeCodeService()
    ...
    func signInWithPhoneOTP(_ payload: PhoneOTPSignInRequest) async throws -> BetterAuthSession {
        try await throttleAuthOperation("signInWithPhoneOTP")
        return try await makeOneTimeCodeService().signInWithPhoneOTP(payload)
    }

    // MARK: - Two Factor

    func enableTwoFactor(_ payload: TwoFactorEnableRequest) async throws -> TwoFactorEnableResponse {
        try await throttleAuthOperation("enableTwoFactor")
        return try await makeTwoFactorService().enableTwoFactor(payload,
    ...
    func verifyTwoFactorTOTP(_ payload: TwoFactorVerifyTOTPRequest) async throws -> BetterAuthSession {
        try await throttleAuthOperation("verifyTwoFactorTOTP")
        return try await makeTwoFactorService().verifyTwoFactorTOTP(payload)
    }

    @discardableResult
    func sendTwoFactorOTP(_ payload: TwoFactorSendOTPRequest = .init()) async throws -> Bool {
        try await throttleAuthOperation("sendTwoFactorOTP")
        return try await makeTwoFactorService().sendTwoFactorOTP(payload)
    }

    @discardableResult
    func verifyTwoFactorOTP(_ payload: TwoFactorVerifyOTPRequest) async throws -> BetterAuthSession {
        try await throttleAuthOperation("verifyTwoFactorOTP")
        return try await makeTwoFactorService().verifyTwoFactorOTP(payload)
    }
    ...
    func verifyTwoFactorRecoveryCode(_ payload: TwoFactorVerifyBackupCodeRequest) async throws
        -> BetterAuthSession
    {
        try await throttleAuthOperation("verifyTwoFactorRecoveryCode")
        return try await makeTwoFactorService().verifyTwoFactorRecoveryCode(payload)
    }
