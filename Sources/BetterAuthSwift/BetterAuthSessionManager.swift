...
public actor BetterAuthSessionManager {
    ...
    public init(
        ...
        self.network = AuthNetworkClient(
    ...
    public nonisolated var onAuthStateChange: AuthEventEmitter { eventEmitter }
    ...
    public func signUpWithEmail(_ payload: EmailSignUpRequest) async throws -> EmailSignUpResult {
        let result: EmailSignUpResult = try await network.post(
            path: configuration.endpoints.emailSignUpPath, body: payload, accessToken: nil
        )
    ...
    public func signInWithEmail(_ payload: EmailSignInRequest) async throws -> BetterAuthSession {
        let session: BetterAuthSession = try await network.post(
            path: configuration.endpoints.emailSignInPath, body: payload, accessToken: nil
        )
        try setSession(session, event: .signedIn)
        return session
    }
    ...
    public func isUsernameAvailable(_ payload: UsernameAvailabilityRequest) async throws -> Bool {
        let response: UsernameAvailabilityResponse = try await network.post(
            path: configuration.endpoints.usernameAvailabilityPath, body: payload, accessToken: nil
        )
    ...
    public func signInWithUsername(_ payload: UsernameSignInRequest) async throws -> BetterAuthSession {
        let session: BetterAuthSession = try await network.post(
            path: configuration.endpoints.usernameSignInPath, body: payload, accessToken: nil
        )
        try setSession(session, event: .signedIn)
        return session
    }
    ...
    public func signInWithApple(_ payload: AppleNativeSignInPayload) async throws -> BetterAuthSession {
        let session: BetterAuthSession = try await network.post(
            path: configuration.endpoints.nativeAppleSignInPath,
            body: payload,
            accessToken: nil
        )
        try setSession(session, event: .signedIn)
    ...
    public func signInWithSocial(_ payload: SocialSignInRequest) async throws -> SocialSignInResult {
        let response: SocialSignInTransportResponse = try await network.post(
            path: configuration.endpoints.socialSignInPath, body: payload, accessToken: nil
        )

        if let session = response.materializedSession {
            try setSession(session, event: .signedIn)
            let signedIn = SocialSignInSuccessResponse(
    ...
    public func signInAnonymously() async throws -> BetterAuthSession {
        let response: SignedInTokenResponse = try await network.post(
            path: configuration.endpoints.anonymousSignInPath, accessToken: nil
        )
    ...
    public func deleteAnonymousUser() async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.deleteAnonymousUserPath, accessToken: current?.session.accessToken
        )
    ...
    public func beginGenericOAuth(_ payload: GenericOAuthSignInRequest) async throws -> GenericOAuthAuthorizationResponse {
        try await network.post(
            path: configuration.endpoints.genericOAuthSignInPath, body: payload, accessToken: nil
        )
    }

    public func linkGenericOAuth(_ payload: GenericOAuthSignInRequest) async throws -> GenericOAuthAuthorizationResponse {
        try await network.post(
            path: configuration.endpoints.genericOAuthLinkPath, body: payload, accessToken: current?.session.accessToken
        )
    }

    @discardableResult
    public func completeGenericOAuth(_ payload: GenericOAuthCallbackRequest) async throws -> BetterAuthSession {
        let session: BetterAuthSession = try await network.get(
            path: oauthCallbackPath(for: payload), accessToken: current?.session.accessToken
        )
        try setSession(session, event: .signedIn)
        return session
    }
    ...
    public func requestPasswordReset(_ payload: ForgotPasswordRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.forgotPasswordPath, body: payload, accessToken: nil
        )
        return response.status
    }
    ...
    public func resetPassword(_ payload: ResetPasswordRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.resetPasswordPath, body: payload, accessToken: nil
        )
        return response.status
    }
    ...
    public func sendVerificationEmail(_ payload: SendVerificationEmailRequest = .init()) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.sendVerificationEmailPath, body: payload, accessToken: current?.session.accessToken
        )
        return response.status
    }
    ...
    public func verifyEmail(_ payload: VerifyEmailRequest) async throws -> VerifyEmailResult {
        let result: VerifyEmailResult = try await network.get(
            path: configuration.endpoints.verifyEmailPath,
    ...
    public func changeEmail(_ payload: ChangeEmailRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.changeEmailPath, body: payload, accessToken: current?.session.accessToken
        )
        return response.status
    }
    ...
    public func updateUser(_ payload: UpdateUserRequest) async throws -> UpdateUserResponse {
        let response: UpdateUserResponse = try await network.post(
            path: configuration.endpoints.updateUserPath, body: payload, accessToken: current?.session.accessToken
        )
        if let user = response.user, let current {
            try setSession(
                BetterAuthSession(session: current.session, user: current.user.merged(with: user)),
                event: .userUpdated
            )
        }
        return response
    ...
    public func changePassword(_ payload: ChangePasswordRequest) async throws -> ChangePasswordResponse {
        let response: ChangePasswordResponse = try await network.post(
            path: configuration.endpoints.changePasswordPath, body: payload, accessToken: current?.session.accessToken
        )
        if payload.revokeOtherSessions == true, let session = response.session {
            ...
            let materializedSession: BetterAuthSession = try await network.get(
                path: configuration.endpoints.currentSessionPath, accessToken: rotatedToken
            )
            try setSession(materializedSession, event: .tokenRefreshed)
        } else if let current {
            try setSession(
                BetterAuthSession(session: current.session, user: current.user.merged(with: response.user)),
                event: .userUpdated
            )
        }
        return response
        ...
        let task = Task { () -> BetterAuthSession in
            if let refreshToken = existingSession.session.refreshToken {
                ...
                return try await network.post(
                    path: endpoints.sessionRefreshPath,
    ...
    public func fetchCurrentSession() async throws -> BetterAuthSession {
        ...
        do {
            let session: BetterAuthSession = try await network.get(
                path: configuration.endpoints.currentSessionPath, accessToken: existingToken
            )
            try setSession(session, event: .tokenRefreshed)
            return session
        } catch {
    ...
    public func listDeviceSessions() async throws -> [BetterAuthDeviceSession] {
        try await network.get(path: configuration.endpoints.listDeviceSessionsPath, accessToken: current?.session.accessToken)
    }

    @discardableResult
    public func setActiveDeviceSession(_ payload: BetterAuthSetActiveDeviceSessionRequest) async throws -> BetterAuthSession {
        let session: BetterAuthSession = try await network.post(
            path: configuration.endpoints.setActiveDeviceSessionPath, body: payload, accessToken: current?.session.accessToken
        )
        try setSession(session, event: .signedIn)
        return session
    }
    ...
    public func revokeDeviceSession(_ payload: BetterAuthRevokeDeviceSessionRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.revokeDeviceSessionPath, body: payload, accessToken: current?.session.accessToken
        )
    ...
    public func listLinkedAccounts() async throws -> [LinkedAccount] {
        try await network.get(path: configuration.endpoints.listLinkedAccountsPath, accessToken: current?.session.accessToken)
    }

    @discardableResult
    public func linkSocialAccount(_ payload: LinkSocialAccountRequest) async throws -> LinkSocialAccountResponse {
        try await network.post(
            path: configuration.endpoints.linkSocialAccountPath, body: payload, accessToken: current?.session.accessToken
        )
    }
    ...
    public func passkeyRegistrationOptions(
        _ request: PasskeyRegistrationOptionsRequest = .init()
    ) async throws -> PasskeyRegistrationOptions {
        try await network.get(
            path: configuration.endpoints.passkeyRegisterOptionsPath,
            queryItems: [
                URLQueryItem(name: "name", value: request.name),
                URLQueryItem(name: "authenticatorAttachment", value: request.authenticatorAttachment),
            ],
    ...
    public func passkeyAuthenticateOptions() async throws -> PasskeyAuthenticationOptions {
        try await network.get(
            path: configuration.endpoints.passkeyAuthenticateOptionsPath, accessToken: current?.session.accessToken
        )
    }
    ...
    public func registerPasskey(_ payload: PasskeyRegistrationRequest) async throws -> Passkey {
        try await network.post(
            path: configuration.endpoints.passkeyRegisterPath, body: payload, accessToken: current?.session.accessToken
        )
    }
    ...
    public func authenticateWithPasskey(_ payload: PasskeyAuthenticationRequest) async throws -> BetterAuthSession {
        let response: SocialSignInTransportResponse = try await network.post(
            path: configuration.endpoints.passkeyAuthenticatePath, body: payload, accessToken: nil
        )
        if let session = response.materializedSession {
            try setSession(session, event: .signedIn)
            return session
    ...
    public func updatePasskey(_ payload: UpdatePasskeyRequest) async throws -> Passkey {
        let response: UpdatePasskeyResponse = try await network.post(
            path: configuration.endpoints.updatePasskeyPath, body: payload, accessToken: current?.session.accessToken
        )
    ...
    public func deletePasskey(_ payload: DeletePasskeyRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.deletePasskeyPath, body: payload, accessToken: current?.session.accessToken
        )
        return response.status
    }
    ...
    public func requestMagicLink(_ payload: MagicLinkRequest) async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.magicLinkSignInPath, body: payload, accessToken: nil
        )
        return response.status
    }
    ...
    public func verifyMagicLink(_ payload: MagicLinkVerifyRequest) async throws -> MagicLinkVerificationResult {
        let result: MagicLinkVerificationResult = try await network.get(
            path: configuration.endpoints.magicLinkVerifyPath,
            queryItems: [
    ...
    public func requestEmailOTP(_ payload: EmailOTPRequest) async throws -> Bool {
        let response: EmailOTPRequestResponse = try await network.post(
            path: configuration.endpoints.emailOTPRequestPath, body: payload, accessToken: nil
        )
    ...
    public func signInWithEmailOTP(_ payload: EmailOTPSignInRequest) async throws -> BetterAuthSession {
        let response: SocialSignInTransportResponse = try await network.post(
            path: configuration.endpoints.emailOTPSignInPath, body: payload, accessToken: nil
        )
        if let session = response.materializedSession {
            try setSession(session, event: .signedIn)
            return session
    ...
    public func verifyEmailOTP(_ payload: EmailOTPVerifyRequest) async throws -> EmailOTPVerifyResult {
        let result: EmailOTPVerifyResult = try await network.post(
            path: configuration.endpoints.emailOTPVerifyPath, body: payload, accessToken: nil
        )
        if case let .signedIn(session) = result {
            try setSession(session, event: .signedIn)
    ...
    public func requestPhoneOTP(_ payload: PhoneOTPRequest) async throws -> Bool {
        let response: PhoneOTPRequestResponse = try await network.post(
            path: configuration.endpoints.phoneOTPRequestPath, body: payload, accessToken: nil
        )
    ...
    public func verifyPhoneNumber(_ payload: PhoneOTPVerifyRequest) async throws -> PhoneOTPVerifyResponse {
        ...
        let response: PhoneOTPVerifyResponse = try await network.post(
            path: configuration.endpoints.phoneOTPVerifyPath, body: payload, accessToken: accessToken
        )
        if let token = response.token, let user = response.user {
            let twoFactorUser = TwoFactorUser(
    ...
    public func signInWithPhoneOTP(_ payload: PhoneOTPSignInRequest) async throws -> BetterAuthSession {
        let response: TwoFactorSessionResponse = try await network.post(
            path: configuration.endpoints.phoneOTPSignInPath, body: payload, accessToken: nil
        )
        let session = try await materializeSession(token: response.token, fallbackUser: response.user)
        try setSession(session, event: .signedIn)
        return session
    ...
    public func enableTwoFactor(_ payload: TwoFactorEnableRequest) async throws -> TwoFactorEnableResponse {
        try await network.post(
            path: configuration.endpoints.twoFactorEnablePath, body: payload, accessToken: current?.session.accessToken
        )
    }
    ...
    public func verifyTwoFactorTOTP(_ payload: TwoFactorVerifyTOTPRequest) async throws -> BetterAuthSession {
        let response: TwoFactorSessionResponse = try await network.post(
            path: configuration.endpoints.twoFactorVerifyTOTPPath, body: payload, accessToken: nil
        )
        let session = try await materializeSession(token: response.token, fallbackUser: response.user)
        try setSession(session, event: .signedIn)
        return session
    ...
    public func sendTwoFactorOTP(_ payload: TwoFactorSendOTPRequest = .init()) async throws -> Bool {
        let response: TwoFactorChallengeStatusResponse = try await network.post(
            path: configuration.endpoints.twoFactorSendOTPPath, body: payload, accessToken: nil
        )
        return response.status
    }
    ...
    public func verifyTwoFactorOTP(_ payload: TwoFactorVerifyOTPRequest) async throws -> BetterAuthSession {
        let response: TwoFactorSessionResponse = try await network.post(
            path: configuration.endpoints.twoFactorVerifyOTPPath, body: payload, accessToken: nil
        )
        let session = try await materializeSession(token: response.token, fallbackUser: response.user)
        try setSession(session, event: .signedIn)
        return session
    ...
    public func verifyTwoFactorRecoveryCode(_ payload: TwoFactorVerifyBackupCodeRequest) async throws -> BetterAuthSession {
        let response: TwoFactorSessionResponse = try await network.post(
            path: configuration.endpoints.twoFactorVerifyBackupCodePath, body: payload, accessToken: nil
        )
        let session = try await materializeSession(token: response.token, fallbackUser: response.user)
        try setSession(session, event: .signedIn)
        return session
    ...
    public func generateTwoFactorRecoveryCodes(password: String) async throws -> [String] {
        ...
        let response: TwoFactorGenerateBackupCodesResponse = try await network.post(
            path: configuration.endpoints.twoFactorGenerateBackupCodesPath,
    ...
    public func revokeSessions() async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.revokeSessionsPath, accessToken: current?.session.accessToken
        )
        try setSession(nil, event: .signedOut)
        return response.status
    }
    ...
    public func revokeOtherSessions() async throws -> Bool {
        let response: BetterAuthStatusResponse = try await network.post(
            path: configuration.endpoints.revokeOtherSessionsPath, accessToken: current?.session.accessToken
        )
        return response.status
    }
    ...
    public func restoreOrRefreshSession() async throws -> BetterAuthSession? {
        ...
        if current.needsRefresh(clockSkew: configuration.clockSkew) {
            do { return try await refreshSession() }
            catch {
                if shouldClearSession(for: error) { try clearSession(event: .sessionExpired) }
                throw error
            }
    ...
    public func signOut(remotely: Bool = true) async throws {
        ...
        if remotely, current != nil {
            _ = try await network.post(
                path: configuration.endpoints.signOutPath, accessToken: current?.session.accessToken
            ) as SignOutResponse
        }
        try setSession(nil, event: .signedOut)
    }
    ...
    public func handle(_ url: URL) async {
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: true),
            ...
            logger?.warning("Could not extract OAuth params from URL: \(url)")
            return
        }

        let pathComponents = components.path.split(separator: "/")
        guard let callbackIndex = pathComponents.firstIndex(of: "callback"),
              callbackIndex + 1 < pathComponents.count else {
            logger?.warning("Could not determine provider from URL: \(url)")
            return
        }
        ...
        do {
            try await completeGenericOAuth(.init(providerId: providerId, code: code, state: state, issuer: issuer))
            logger?.info("OAuth callback handled for provider: \(providerId)")
        } catch {
    ...
    private func materializeSession(token: String, fallbackUser: TwoFactorUser) async throws -> BetterAuthSession {
        ...
        let session: BetterAuthSession = try await network.get(
            path: configuration.endpoints.currentSessionPath, accessToken: token
        )
    ...
    private func materializeSession(token: String, fallbackUser: BetterAuthSession.User) async throws -> BetterAuthSession {
        let previous = current
        let session: BetterAuthSession = try await network.get(
            path: configuration.endpoints.currentSessionPath, accessToken: token
        )
...
private struct RevokeSessionRequest: Encodable, Sendable {
    let token: String
}
