          ...
          genericOAuth({
            config: [
              ...
              },
              {
                ...
                accountIssuer: getAppleAuthBaseURL(env),
                authorizationUrl: `${getAppleAuthBaseURL(env)}/auth/authorize`,
                tokenUrl: `${getAppleAuthBaseURL(env)}/auth/token`,
                clientId: env.APPLE_CLIENT_ID,
                clientSecret: appleClientSecret ?? 'fixture-apple-client-secret',
                scopes: ['email', 'name'],
                async getUserInfo(tokens) {
                  if (!tokens.idToken) {
                    return null;
                  }
                  const profile = decodeJwt<Record<string, unknown>>(tokens.idToken);
                  return {
                    id: String(profile.sub ?? ''),
                    email: String(profile.email ?? ''),
                    emailVerified: profile.email_verified === true || profile.email_verified === 'true',
                    name: String(profile.name ?? 'Apple Emulator User'),
                  };
                },
              },
            ],
