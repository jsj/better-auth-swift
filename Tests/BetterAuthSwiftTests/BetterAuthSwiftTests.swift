...
struct BetterAuthSwiftTests {
    ...
    func loadStoredSessionUsesStoreWithoutActorHop() throws {
        let session = BetterAuthSession(
            session: .init(id: "session-1", userId: "user-1", accessToken: "token"),
            user: .init(id: "user-1", email: "test@example.com")
        )
        let store = InMemorySessionStore()
        try store.saveSession(session, for: "test-key")

        let manager = BetterAuthSessionManager(
    ...
    func refreshSessionIfNeededReturnsCurrentSessionWhenFresh() async throws {
        ...
        let manager = BetterAuthSessionManager(
            ...
            sessionStore: InMemorySessionStore(),
            transport: MockTransport { _ in
                Issue.record("Network should not be used for a fresh in-memory session")
                return emptyResponse(for: URLRequest(url: URL(string: "https://example.com")!))
            }
        )
    ...
    func authorizedRequestRefreshesExpiringSession() async throws {
        let expiring = BetterAuthSession(
            session: .init(
                id: "session-1",
                userId: "user-1",
        ...
        let refreshed = BetterAuthSession(
            session: .init(
                id: "session-2",
                userId: "user-1",
    ...
    func appleNonceHashIsStable() {
        ...
        #expect(AppleSignInSupport.sha256("nonce") == "78377b525757b494427f89014f97d79928f3938d14eb51e20fb5dec9834eb304")
    ...
    func unauthenticatedRequestsIncludeConfiguredOriginHeader() async throws {
        ...
        let _: String? = try await client.requests.sendJSON(
    ...
    func requestClientRetriesAfterUnauthorized() async throws {
        ...
        let transport = SequencedMockTransport([
            .response(statusCode: 401, jsonObject: ["error": "expired"]),
        ...
        try await client.auth.updateSession(
            BetterAuthSession(
                session: .init(
                    id: "session-1",
                    userId: "user-1",
    ...
    func requestClientRetriesOnlyOnceAfterUnauthorized() async throws {
        ...
        let transport = SequencedMockTransport([
            .response(statusCode: 401, jsonObject: ["error": "expired"]),
            .response(statusCode: 200, encodable: refreshed),
            .response(statusCode: 401, jsonObject: ["error": "still unauthorized"])
        ])
    ...
    func fetchCurrentSessionSynchronizesMemoryAndPersistence() async throws {
        ...
        let manager = BetterAuthSessionManager(
            ...
            transport: MockTransport { request in
                #expect(request.url?.path == "/api/auth/get-session")
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer old-token")
                return response(for: request, statusCode: 200, data: try encodeJSON(fetched))
            }
        )
    ...
    func sessionStorageSharedFactoryUsesProvidedAccessGroup() {
        let storage = BetterAuthConfiguration.SessionStorage.shared(
            accessGroup: "group.sh.jsj.better-auth"
        )
    ...
    func tokenRotationPersistsAcrossRefreshAndLaterRestore() async throws {
        ...
        let store = InMemorySessionStore()
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: rotated),
            .response(statusCode: 200, encodable: ProtectedResponse(email: "test@example.com"))
        ])
        ...
        let rehydratedManager = BetterAuthSessionManager(
            configuration: BetterAuthConfiguration(
                baseURL: URL(string: "https://example.com")!,
                storage: .init(key: "test-key"),
                clockSkew: 60
    ...
    func listSessionsDecodesStableSessionInventoryModels() async throws {
        ...
        let client = BetterAuthClient(
            ...
            transport: MockTransport { request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(sessions))
            }
        )

        try await client.auth.updateSession(
            BetterAuthSession(
                session: .init(id: "current-session", userId: "user-1", accessToken: "current-token"),
                user: .init(id: "user-1", email: "test@example.com")
            )
        )
    ...
    func revokeNonCurrentSessionPreservesLocalCurrentSession() async throws {
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: BetterAuthStatusResponse(status: true))
        ])
    ...
    func deviceSessionSurfacesDecodeSetActiveAndRevokeSemantics() async throws {
        ...
        let transport = SequencedMockTransport([
            .handler { request in
                ...
                #expect(request.httpMethod == "GET")
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer current-token")
                return response(for: request, statusCode: 200, data: try encodeJSON(listedSessions))
            },
            .handler { request in
                #expect(request.url?.path == "/api/auth/multi-session/set-active")
                #expect(request.httpMethod == "POST")
                let payload = try JSONDecoder().decode(BetterAuthSetActiveDeviceSessionRequest.self, from: try #require(request.httpBody))
                #expect(payload.sessionToken == "device-token-2")
                return response(for: request, statusCode: 200, data: try encodeJSON(activeSession))
            },
            .handler { request in
                #expect(request.url?.path == "/api/auth/multi-session/revoke")
                #expect(request.httpMethod == "POST")
                let payload = try JSONDecoder().decode(BetterAuthRevokeDeviceSessionRequest.self, from: try #require(request.httpBody))
                #expect(payload.sessionToken == "device-token-1")
                return response(for: request, statusCode: 200, data: try encodeJSON(BetterAuthStatusResponse(status: true)))
            },
        ])

        let store = InMemorySessionStore()
        let client = BetterAuthClient(
            configuration: BetterAuthConfiguration(baseURL: URL(string: "https://example.com")!),
            sessionStore: store,
            transport: transport
        )
        try await client.auth.updateSession(
            BetterAuthSession(
                session: .init(id: "session-current", userId: "user-1", accessToken: "current-token"),
                user: .init(id: "user-1", email: "test@example.com")
            )
        )
    ...
    func jwtAndJWKSDecodeFromWorkerSurfaces() async throws {
        let transport = SequencedMockTransport([
            .handler { request in
                ...
                #expect(request.httpMethod == "GET")
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer current-token")
                return response(for: request, statusCode: 200, data: try encodeJSON(BetterAuthJWT(token: "jwt-token-value")))
            },
            .handler { request in
    ...
    func twoFactorEnableAndTOTPVerificationPersistNativeSession() async throws {
        let current = BetterAuthSession(
            session: .init(id: "primary-session", userId: "user-2fa", accessToken: "primary-token"),
            user: .init(id: "user-2fa", email: "twofactor@example.com", name: "2FA User")
        )
        let store = InMemorySessionStore()
        let nativeSession = BetterAuthSession(
            session: .init(
                id: "twofactor-session",
                userId: "user-2fa",
        ...
        let transport = SequencedMockTransport([
            .handler { request in
                ...
                let payload = try JSONDecoder().decode(TwoFactorEnableRequest.self, from: try #require(request.httpBody))
                ...
                return response(
                    ...
                    data: try encodeJSON(
                        TwoFactorEnableResponse(
                            totpURI: "otpauth://totp/Better%20Auth%20Swift:twofactor@example.com?secret=ABC123",
            ...
            .handler { request in
                ...
                let payload = try JSONDecoder().decode(TwoFactorVerifyTOTPRequest.self, from: try #require(request.httpBody))
            ...
            .handler { request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(nativeSession))
            },
        ])
    ...
    func twoFactorOTPAndRecoveryCodeFlowsPersistSessionAndExposeFailures() async throws {
        let otpSession = BetterAuthSession(
            session: .init(
                id: "otp-session",
                userId: "user-2fa",
        ...
        let recoverySession = BetterAuthSession(
            session: .init(
                id: "recovery-session",
                userId: "user-2fa",
        ...
        let client = BetterAuthClient(
            ...
            transport: SequencedMockTransport([
                .response(statusCode: 200, encodable: TwoFactorChallengeStatusResponse(status: true)),
                .response(
                    statusCode: 200,
                    encodable: TwoFactorSessionResponse(
                        token: "otp-token",
                        user: .init(
                            id: "user-2fa",
                            email: "twofactor@example.com",
                ...
                .response(statusCode: 200, encodable: otpSession),
                .response(
                    statusCode: 200,
                    encodable: TwoFactorSessionResponse(
                ...
                .response(statusCode: 200, encodable: recoverySession),
                .response(statusCode: 400, jsonObject: ["code": "INVALID_BACKUP_CODE", "message": "Invalid backup code"]),
                .response(statusCode: 401, jsonObject: ["code": "INVALID_BACKUP_CODE", "message": "Invalid backup code"]),
            ])
        )
        ...
        await assertRequestFailedJSON(statusCode: 400, expectedJSON: [
        ...
        await assertRequestFailedJSON(statusCode: 401, expectedJSON: [
            "code": "INVALID_BACKUP_CODE",
            "message": "Invalid backup code",
        ]) {
    ...
    func passkeyAuthenticateOptionsWorkWhileSignedOut() async throws {
        let expected = PasskeyAuthenticationOptions(
        ...
        let client = BetterAuthClient(
            ...
            sessionStore: InMemorySessionStore(),
            transport: MockTransport { request in
                #expect(request.url?.path == "/api/auth/passkey/generate-authenticate-options")
                #expect(request.httpMethod == "GET")
                #expect(request.value(forHTTPHeaderField: "Authorization") == nil)
                return response(for: request, statusCode: 200, data: try encodeJSON(expected))
            }
        )
    ...
    func passkeyRegistrationAndManagementFlowsUseAuthenticatedRoutes() async throws {
        ...
        let current = BetterAuthSession(
            session: .init(id: "session-1", userId: "user-1", accessToken: "passkey-token"),
        ...
        let options = PasskeyRegistrationOptions(
            ...
            user: .init(id: "user-handle", name: "passkey@example.com", displayName: "passkey@example.com"),
            pubKeyCredParams: [.init(type: "public-key", alg: -7)],
            timeout: 60000,
            excludeCredentials: [.init(id: "credential-id", type: "public-key", transports: ["internal"])],
            authenticatorSelection: .init(authenticatorAttachment: "platform", requireResidentKey: nil, residentKey: "preferred", userVerification: "preferred"),
            attestation: "none"
        )
        ...
        let transport = SequencedMockTransport([
            .handler { request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(options))
            },
            .handler { request in
                #expect(request.url?.path == "/api/auth/passkey/verify-registration")
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer passkey-token")
                let payload = try JSONDecoder().decode(PasskeyRegistrationRequest.self, from: try #require(request.httpBody))
                #expect(payload.name == "MacBook")
                #expect(payload.response.id == "credential-id")
                return response(for: request, statusCode: 200, data: try encodeJSON(createdPasskey))
            },
            .handler { request in
            ...
            .handler { request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(UpdatePasskeyResponse(passkey: renamedPasskey)))
            },
            .handler { request in
        ...
        let registerOptions = try await client.auth.passkeyRegistrationOptions(
            .init(name: "MacBook", authenticatorAttachment: "platform")
        )
        #expect(registerOptions == options)

        let created = try await client.auth.registerPasskey(
            .init(
                response: .init(
                    id: "credential-id",
    ...
    func passkeyAuthenticationPersistsSessionAndSupportsRestore() async throws {
        let signedInSession = BetterAuthSession(
            session: .init(
                id: "passkey-session",
                userId: "user-1",
            ...
            user: .init(id: "user-1", email: "passkey@example.com", name: "Passkey User")
        )
        let protectedPayload = ProtectedResponse(email: "passkey@example.com")
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: SocialSignInTransportResponse(
                redirect: false,
                token: signedInSession.session.accessToken,
        ...
        let session = try await client.auth.authenticateWithPasskey(
            .init(
                response: .init(
                    id: "credential-id",
                    rawId: "credential-raw",
    ...
    func passkeyVerificationFailuresRemainExplicitForContinuityBreaks() async throws {
        let client = BetterAuthClient(
            ...
            sessionStore: InMemorySessionStore(),
            transport: SequencedMockTransport([
                .response(statusCode: 400, jsonObject: ["code": "CHALLENGE_NOT_FOUND", "message": "Challenge not found"]),
                .response(statusCode: 400, jsonObject: ["code": "AUTHENTICATION_FAILED", "message": "Authentication failed"]),
                .response(statusCode: 400, jsonObject: ["code": "FAILED_TO_VERIFY_REGISTRATION", "message": "Failed to verify registration"]),
            ])
        )
        ...
        await assertRequestFailedJSON(statusCode: 400, expectedJSON: [
            ...
            _ = try await client.auth.authenticateWithPasskey(
                .init(
                    response: .init(
                        id: "credential-id",
                        rawId: "credential-raw",
            ...
            _ = try await client.auth.registerPasskey(
                .init(
                    response: .init(
                        ...
                        rawId: "credential-raw",
                        response: .init(
                            clientDataJSON: "stale-registration",
                            attestationObject: "attestation"
                        )
                    )
                )
    ...
    func socialSignInReturnsAuthorizationURLWithoutPersistingSession() async throws {
        let transport = MockTransport { request in
            ...
            return response(
                for: request,
                statusCode: 200,
                data: try encodeJSON(SocialAuthorizationResponse(
        ...
        let result = try await client.auth.signInWithSocial(
            SocialSignInRequest(provider: "google", disableRedirect: true)
        )
    ...
    func anonymousSignInMaterializesAndPersistsNativeSession() async throws {
        let transport = SequencedMockTransport([
            .response(statusCode: 200, jsonObject: [
                "token": "anon-token",
                "user": [
                    "id": "anon-user",
    ...
    func genericOAuthInitiationAndCompletionMaterializeNativeSession() async throws {
        ...
        let transport = MockTransport { request in
            if request.url?.path == "/api/auth/sign-in/oauth2" {
                #expect(request.httpMethod == "POST")
                let payload = try JSONDecoder().decode(GenericOAuthSignInRequest.self, from: try #require(request.httpBody))
                #expect(payload.providerId == "fixture-generic")
                #expect(payload.disableRedirect == true)
                return response(
                    for: request,
                    statusCode: 200,
                    data: try encodeJSON(GenericOAuthAuthorizationResponse(
                        url: "https://fixture-oauth.example.com/oauth/authorize?state=fixture-state",
                        redirect: false
                    ))
                )
            }
            ...
            if request.url?.path == "/api/auth/oauth2/callback/fixture-generic" {
                ...
                return response(
                    for: request,
                    statusCode: 200,
                    data: try encodeJSON(BetterAuthSession(
                        session: .init(id: "session-oauth", userId: "oauth-user", accessToken: "oauth-token"),
                        user: .init(id: "oauth-user", email: "oauth@example.com", name: "OAuth User")
                    ))
                )
            }
        ...
        let authURL = try await client.auth.beginGenericOAuth(
            GenericOAuthSignInRequest(
        ...
        let session = try await client.auth.completeGenericOAuth(
            GenericOAuthCallbackRequest(
                providerId: "fixture-generic",
                code: "fixture-code",
                state: "fixture-state"
            )
    ...
    func listLinkedAccountsDecodesStableModels() async throws {
        ...
        let client = BetterAuthClient(
            ...
            transport: MockTransport { request in
                ...
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer current-token")
                return response(for: request, statusCode: 200, data: try encodeJSON(linkedAccounts))
            }
        )
    ...
    func genericOAuthLinkUsesAuthenticatedRoute() async throws {
        ...
        let result = try await client.auth.linkGenericOAuth(
            GenericOAuthSignInRequest(
                providerId: "fixture-generic",
                callbackURL: "betterauth://oauth/success",
                disableRedirect: true
    ...
    func genericOAuthLinkCallbackReconcilesExistingSessionWhenCallbackDoesNotRotateBearer() async throws {
        let existingSession = BetterAuthSession(
            session: .init(id: "current-session", userId: "user-1", accessToken: "current-token"),
            user: .init(id: "user-1", email: "current@example.com", name: "Current User")
        )
        let reconciledSession = BetterAuthSession(
            session: .init(id: "current-session", userId: "user-1", accessToken: "current-token"),
            user: .init(id: "user-1", email: "current@example.com", name: "Current User", username: "linked-user", displayUsername: "Linked User")
        )
        let store = InMemorySessionStore()
        let client = BetterAuthClient(
            configuration: BetterAuthConfiguration(baseURL: URL(string: "https://example.com")!),
            ...
            transport: MockTransport { request in
                ...
                #expect(components?.queryItems?.first(where: { $0.name == "code" })?.value == "fixture-code")
                #expect(components?.queryItems?.first(where: { $0.name == "state" })?.value == "fixture-state")
                return response(for: request, statusCode: 200, data: try encodeJSON(reconciledSession))
            }
        )
    ...
    func linkSocialAccountDecodesSuccessResponse() async throws {
        let client = BetterAuthClient(
            ...
            transport: MockTransport { request in
                ...
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer current-token")
                let payload = try JSONDecoder().decode(LinkSocialAccountRequest.self, from: try #require(request.httpBody))
                #expect(payload.provider == "google")
                #expect(payload.idToken?.token == "valid-google-token")
                return response(
                    for: request,
                    statusCode: 200,
                    data: try encodeJSON(LinkSocialAccountResponse(url: "", redirect: false, status: true))
                )
            }
        )
        ...
        let result = try await client.auth.linkSocialAccount(
    ...
    func linkSocialAccountPreservesPolicyFailureSemantics() async throws {
        ...
        do {
            _ = try await client.auth.linkSocialAccount(
                LinkSocialAccountRequest(
                    provider: "google",
    ...
    func requestPasswordResetUsesConfiguredEndpoint() async throws {
        ...
        let status = try await client.auth.requestPasswordReset(
            ForgotPasswordRequest(email: "reset@example.com", redirectTo: "https://app.example.com/reset")
        )
    ...
    func resetPasswordUsesConfiguredEndpoint() async throws {
        ...
        let status = try await client.auth.resetPassword(
            ResetPasswordRequest(token: "reset-token", newPassword: "new-password-123")
        )
    ...
    func sendVerificationEmailUsesConfiguredEndpointAndCurrentBearer() async throws {
        let transport = MockTransport { request in
            ...
            #expect(request.httpMethod == "POST")
            #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer current-token")
            let payload = try JSONDecoder().decode(SendVerificationEmailRequest.self, from: try #require(request.httpBody))
        ...
        let status = try await client.auth.sendVerificationEmail(
            SendVerificationEmailRequest(callbackURL: "https://app.example.com/verify")
        )
    ...
    func verifyEmailWithAutoSignInPersistsNativeSession() async throws {
        let verifiedSession = BetterAuthSession(
            session: .init(
                id: "verified-session",
                userId: "user-1",
            ...
            user: .init(id: "user-1", email: "verified@example.com", name: "Verified User")
        )
        let transport = MockTransport { request in
            #expect(request.url?.path == "/api/auth/verify-email")
            #expect(request.httpMethod == "GET")
            ...
            return response(
                ...
                statusCode: 200,
                data: try encodeJSON(
                    SocialSignInTransportResponse(
                        redirect: false,
                        token: verifiedSession.session.accessToken,
    ...
    func changeEmailUsesConfiguredEndpointAndCurrentBearer() async throws {
        ...
        let status = try await client.auth.changeEmail(
            ChangeEmailRequest(newEmail: "next@example.com", callbackURL: "https://app.example.com/settings")
        )
    ...
    func changePasswordSurfacesBackendPolicyFailures() async throws {
        ...
        let location = SourceLocation(fileID: #fileID, filePath: #filePath, line: #line + 1, column: #column)
        do {
            _ = try await client.auth.changePassword(
    ...
    func changePasswordWithRevokeOtherSessionsMaterializesReplacementSession() async throws {
        ...
        let replacementSession = BetterAuthSession(
            session: .init(
        ...
        let transport = SequencedMockTransport([
            .handler({ request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(ChangePasswordResponse(
                    token: "rotated-token",
                    user: .init(
                        id: "user-1",
                        email: "user@example.com",
            ...
            .handler({ request in
                ...
                #expect(request.httpMethod == "GET")
                #expect(request.value(forHTTPHeaderField: "Authorization") == "Bearer rotated-token")
                return response(for: request, statusCode: 200, data: try encodeJSON(replacementSession))
            }),
        ])
        ...
        let result = try await client.auth.changePassword(
            .init(currentPassword: "old-password", newPassword: "new-password-123", revokeOtherSessions: true)
        )
    ...
    func emailSignUpPersistsNativeSessionAndSupportsRestore() async throws {
        let signedUpSession = BetterAuthSession(
            session: .init(
                id: "session-sign-up",
                userId: "user-1",
            ...
            user: .init(id: "user-1", email: "signup@example.com", name: "Sign Up User")
        )

        let protectedPayload = ProtectedResponse(email: "signup@example.com")
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: signedUpSession),
            .response(statusCode: 200, encodable: protectedPayload)
        ])
        ...
        let result = try await client.auth.signUpWithEmail(
            .init(email: "signup@example.com", password: "password123", name: "Sign Up User")
        )
        guard case let .signedIn(session) = result else {
    ...
    func emailSignUpReturnsExplicitVerificationHeldResultWithoutPersistingSession() async throws {
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: VerificationHeldEmailSignUp(
                requiresVerification: true,
                user: .init(id: "user-held", email: "held@example.com", name: "Held User")
            ))
        ])
        ...
        let result = try await client.auth.signUpWithEmail(
            .init(email: "held@example.com", password: "password123", name: "Held User")
        )
    ...
    func emailSignUpReturnsExplicitSignedUpResultWhenAutoSignInIsDisabled() async throws {
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: SuccessfulEmailSignUp(
        ...
        let result = try await client.auth.signUpWithEmail(
            .init(email: "signed-up@example.com", password: "password123", name: "Signed Up User")
        )
    ...
    func emailSignUpNormalizesDuplicateEmailIntoEnumerationSafeSignedUpResult() async throws {
        ...
        let result = try await client.auth.signUpWithEmail(
            EmailSignUpRequest(email: "duplicate@example.com", password: "password123", name: "Duplicate User")
        )
    ...
    func emailSignInPersistsNativeSessionAndSupportsRestore() async throws {
        let signedInSession = BetterAuthSession(
            ...
            user: .init(id: "user-1", email: "signin@example.com", name: "Sign In User")
        )

        let protectedPayload = ProtectedResponse(email: "signin@example.com")
        let transport = SequencedMockTransport([
            .response(statusCode: 200, encodable: signedInSession),
            .response(statusCode: 200, encodable: protectedPayload)
        ])
        ...
        let session = try await client.auth.signInWithEmail(
            .init(email: "signin@example.com", password: "password123")
        )
        #expect(session.session.id == signedInSession.session.id)
        #expect(session.session.accessToken == signedInSession.session.accessToken)
        #expect(session.user.email == signedInSession.user.email)
    ...
    func emailSignInPreservesExplicitUnverifiedEmailFailure() async throws {
        ...
        let location = SourceLocation(fileID: #fileID, filePath: #filePath, line: #line + 1, column: #column)
        do {
            _ = try await client.auth.signInWithEmail(
                EmailSignInRequest(email: "unverified@example.com", password: "password123")
            )
            Issue.record("Expected BetterAuthError.requestFailed", sourceLocation: location)
        } catch let BetterAuthError.requestFailed(statusCode, _, _, response) {
    ...
    func usernameAvailabilityUsesConfiguredEndpointAndPreservesNormalizationSemantics() async throws {
        let transport = SequencedMockTransport([
            .handler({ request in
                ...
                #expect(request.httpMethod == "POST")
                let payload = try JSONDecoder().decode(UsernameAvailabilityRequest.self, from: try #require(request.httpBody))
                #expect(payload.username == "PRIORITY_USER")
                return response(for: request, statusCode: 200, data: try encodeJSON(UsernameAvailabilityResponse(available: false)))
            }),
            .handler({ request in
                #expect(request.url?.path == "/api/auth/is-username-available")
                let payload = try JSONDecoder().decode(UsernameAvailabilityRequest.self, from: try #require(request.httpBody))
                #expect(payload.username == "fresh_user")
                return response(for: request, statusCode: 200, data: try encodeJSON(UsernameAvailabilityResponse(available: true)))
            }),
        ])
    ...
    func usernameSignInPersistsNativeSessionAndSupportsRestore() async throws {
        ...
        let session = try await client.auth.signInWithUsername(
            .init(username: "Custom_User", password: "password123")
        )
        #expect(session.session.id == signedInSession.session.id)
        #expect(session.session.accessToken == signedInSession.session.accessToken)
        #expect(session.user.email == signedInSession.user.email)
    ...
    func usernameSignInPreservesInvalidCredentialFailure() async throws {
        let client = BetterAuthClient(
            ...
            sessionStore: InMemorySessionStore(),
            transport: SequencedMockTransport([
                .response(statusCode: 401, jsonObject: ["code": "INVALID_USERNAME_OR_PASSWORD", "message": "Invalid username or password"]),
            ])
        )
    ...
    func emailSignUpPreservesUsernameFieldsAndNormalizationSemantics() async throws {
        ...
        let transport = SequencedMockTransport([
            .handler({ request in
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(signedUpSession))
            }),
        ])
    ...
    func updateUserPreservesUsernameFieldsAndCurrentSessionState() async throws {
        ...
        let updatedUser = BetterAuthSession.User(
            id: "user-username",
            email: "username@example.com",
            name: "Updated Name",
        ...
        let transport = MockTransport { request in
            ...
            return response(for: request, statusCode: 200, data: try encodeJSON(UpdateUserResponse(status: true, user: updatedUser)))
        }
        ...
        try await client.auth.updateSession(existingSession)
        let result = try await client.auth.updateUser(
            .init(name: "Updated Name", username: "Priority_User", displayUsername: "Priority Display Name")
        )
    ...
    func updateUserPreservesDuplicateUsernameFailureSemantics() async throws {
        let client = BetterAuthClient(
            ...
            sessionStore: InMemorySessionStore(),
            transport: SequencedMockTransport([
                .response(statusCode: 400, jsonObject: ["code": "USERNAME_IS_ALREADY_TAKEN", "message": "Username is already taken"]),
            ])
        )

        try await client.auth.updateSession(
            BetterAuthSession(
                session: .init(id: "existing-session", userId: "user-username", accessToken: "current-token"),
                user: .init(id: "user-username", email: "username@example.com")
            )
        )
    ...
    func magicLinkRequestEncodesNativeContextFields() async throws {
        ...
        let status = try await client.auth.requestMagicLink(
            MagicLinkRequest(
    ...
    func magicLinkVerificationPersistsSessionAndSupportsRestore() async throws {
        let verifiedSession = BetterAuthSession(
            ...
            user: .init(id: "user-magic", email: "magic@example.com", name: "Magic User")
        )
        ...
        let result = try await client.auth.verifyMagicLink(
            MagicLinkVerifyRequest(token: "magic-token-value")
        )
    ...
    func magicLinkVerificationFailureRemainsInspectable() async throws {
        let transport = SequencedMockTransport([
            .response(
                statusCode: 400,
                encodable: MagicLinkVerificationResult.failure(
                    MagicLinkFailure(
                        error: "EXPIRED_TOKEN",
                        status: 302,
        ...
        let location = SourceLocation(fileID: #fileID, filePath: #filePath, line: #line + 1, column: #column)
        do {
            _ = try await client.auth.verifyMagicLink(
    ...
    func emailOTPRequestUsesConfiguredEndpoint() async throws {
        let transport = MockTransport { request in
            ...
            return response(for: request, statusCode: 200, data: try encodeJSON(EmailOTPRequestResponse(success: true)))
        }
    ...
    func emailOTPSignInPersistsNativeSessionAndSupportsRestore() async throws {
        ...
        let session = try await client.auth.signInWithEmailOTP(
            .init(email: "otp@example.com", otp: "123456", name: "OTP User")
        )
        #expect(session.session.id == signedInSession.session.id)
        #expect(session.session.accessToken == signedInSession.session.accessToken)
        #expect(session.user.email == signedInSession.user.email)
    ...
    func emailOTPSignInPreservesStableFailureContracts() async throws {
        let client = BetterAuthClient(
            ...
            sessionStore: InMemorySessionStore(),
            transport: SequencedMockTransport([
                .response(statusCode: 400, jsonObject: ["code": "INVALID_OTP", "message": "Invalid OTP"]),
                .response(statusCode: 400, jsonObject: ["code": "OTP_EXPIRED", "message": "OTP expired"]),
                .response(statusCode: 403, jsonObject: ["code": "TOO_MANY_ATTEMPTS", "message": "Too many attempts"]),
            ])
        )
        ...
        await assertRequestFailedJSON(statusCode: 403, expectedJSON: [
    ...
    func emailOTPVerifyUpdatesCurrentUserWithoutPersistingNewSession() async throws {
        ...
        let verifiedUser = BetterAuthSession.User(
            id: "user-otp",
            email: "otp@example.com",
        ...
        let client = BetterAuthClient(
            ...
            sessionStore: store,
            transport: SequencedMockTransport([
                .response(statusCode: 200, encodable: EmailOTPVerifyResult.verified(verifiedUser))
            ])
        )
    ...
    func emailOTPVerifyWithAutoSignInPersistsNativeSession() async throws {
        let verifiedSession = BetterAuthSession(
            session: .init(
                id: "otp-verified-session",
                userId: "user-otp",
                accessToken: "verified-otp-token",
                expiresAt: Date().addingTimeInterval(3600)
            ),
            user: .init(id: "user-otp", email: "otp@example.com", name: "Verified OTP User")
        )
    ...
    func phoneOTPRequestUsesConfiguredEndpoint() async throws {
        let transport = MockTransport { request in
            ...
            return response(for: request, statusCode: 200, data: try encodeJSON(PhoneOTPRequestResponse(message: "code sent")))
        }
    ...
    func phoneNumberVerifyUsesAuthenticatedRouteAndUpdatesReturnedUserWhenUpdatingPhoneNumber() async throws {
        let existingSession = BetterAuthSession(
            ...
            user: .init(id: "user-phone", email: "phone@example.com", name: "Phone User")
        )
        ...
        try await client.auth.updateSession(existingSession)
        let verified = try await client.auth.verifyPhoneNumber(PhoneOTPVerifyRequest(phoneNumber: "+15555550123", code: "123456", disableSession: true, updatePhoneNumber: true))
        #expect(verified == PhoneOTPVerifyResponse(status: true, user: BetterAuthSession.User(id: "user-phone", email: "phone@example.com", name: "Phone User")))
    ...
    func phoneNumberVerifyUsesPublicRouteWhenNotUpdatingPhoneNumber() async throws {
        ...
        let verified = try await client.auth.verifyPhoneNumber(PhoneOTPVerifyRequest(phoneNumber: "+15555550123", code: "123456"))
    ...
    func phoneNumberVerifyMaterializesSessionWhenSessionCreationIsEnabled() async throws {
        let fetchedSession = BetterAuthSession(
            session: .init(
                id: "phone-session",
                userId: "user-phone",
                accessToken: "phone-token",
    ...
    func phoneNumberSignInMaterializesPersistedNativeSession() async throws {
        ...
        let session = try await client.auth.signInWithPhoneOTP(.init(phoneNumber: "+15555550123", password: "password123"))
    ...
    func appleSignInUsesConfiguredNativeRouteAndPersistsSession() async throws {
        let signedInSession = BetterAuthSession(
            session: .init(
                id: "session-apple-sign-in",
                userId: "user-apple",
                accessToken: "apple-token",
                refreshToken: "apple-refresh-token",
                expiresAt: Date().addingTimeInterval(3600)
            ),
            user: .init(id: "user-apple", email: "apple@example.com", name: "Apple User")
        )
        ...
        let client = BetterAuthClient(
            ...
            transport: MockTransport { request in
                ...
                let payload = try JSONDecoder().decode(AppleNativeSignInPayload.self, from: try #require(request.httpBody))
                ...
                return response(for: request, statusCode: 200, data: try encodeJSON(signedInSession))
            }
        )
    ...
    func appleSignInAllowsRepeatAuthorizationWithoutFirstUseProfileHints() async throws {
        let signedInSession = BetterAuthSession(
            session: .init(
                id: "session-repeat-apple-sign-in",
                userId: "user-apple",
    ...
    func appleSignInUsesConfiguredNativeRouteWithoutSocialFallback() async throws {
        ...
        let client = BetterAuthClient(
            configuration: BetterAuthConfiguration(
                baseURL: URL(string: "https://example.com")!,
                storage: .init(key: "test-key"),
                endpoints: .init(nativeAppleSignInPath: "/api/auth/custom-apple/native", socialSignInPath: "/api/auth/sign-in/social"),
                requestOrigin: "app://snoozy"
            ),
            sessionStore: store,
            transport: MockTransport { request in
...
private actor SequencedMockTransport: BetterAuthTransport {
    enum Entry {
        ...
        static func response<Response: Encodable>(statusCode: Int, encodable: Response) -> Entry {
...
private func assertRequestFailed<R>(
...
private func assertRequestFailedJSON<R>(
    statusCode expectedStatusCode: Int,
